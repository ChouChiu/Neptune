...
这是Telegram群组管理机器人Neptune的README，教你如何使用涅普涅普来管理群组。

**Neptune** - 穿越到 Telegram 群组的紫耀之都守护女神，涅普涅普用 Go 写的群管理Bot！单二进制部署，SQLite 存储。

虽然平时懒散，但处理入群验证、自动回复关键词时，涅普可是很可靠的！（大概）
...
- **Web 管理面板** - 用 Telegram Login Widget 认证的网页后台（`域名/admin`），支持举报审核和警告管理，指挥官的作战指挥室！
- **GitHub Release 通知** - 自动接收 Kazumi 仓库新 Release 并发送到 Telegram 频道，新版本的情报交给人家吧！发布完了记得请涅普吃布丁哦？
- **权限控制** - 所有管理命令仅限管理员使用，越权什么的不可以哦！涅普可是有原则的！（才不是因为懒呢）
...
- **每日限额**：你每天可以和涅普聊天15次，管理员大人没有限制哦！（管理员大人真是太伟大了！涅普给你点赞！）
- **记忆机制**：群内共享上下文，保留最近 50 条消息（滚动 7 天窗口），涅普记性好！（大概）
...
4. 使用各种命令配置群组功能，就这样，简单对吧？涅普都能学会！

## 开发 (for Developers)

详见 [贡献指南](CONTRIBUTING.md)。（涅普不太懂这些，但指挥官应该没问题吧？）
